...
export class AuthClient {
    ...
    seedOperator(): Promise<FileResponse> {
        let url_ = this.baseUrl + "/api/auth/seed-operator";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processSeedOperator(_response);
        });
    }

    protected processSeedOperator(response: Response): Promise<FileResponse> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200 || status === 206) {
            const contentDisposition = response.headers ? response.headers.get("content-disposition") : undefined;
...
export class CommandsClient {
    ...
    getHistory(farmId: string | null | undefined, turbineId: string | null | undefined, take: number | undefined): Promise<FileResponse> {
...
export class TurbineControlClient {
    private http: { fetch(url: RequestInfo, init?: RequestInit): Promise<Response> };
    private baseUrl: string;
    protected jsonParseReviver: ((key: string, value: any) => any) | undefined = undefined;
    ...
    sendCommand(farmId: string, turbineId: string, dto: TurbineCommandDto): Promise<FileResponse> {
        let url_ = this.baseUrl + "/api/farms/{farmId}/turbines/{turbineId}/command";
        if (farmId === undefined || farmId === null)
